...
class _UpdateCenterPageState extends State<UpdateCenterPage> {
  ...
  Future<void> _check() async {
    ...
    try {
      ...
      if (mounted) setState(() => _error = 'تعذر الاتصال بخدمة التحديث. تحقق من الإنترنت ثم أعد المحاولة.');
    } finally {
  ...
  Future<void> _download() async {
    ...
    try {
      ...
      if (mounted) setState(() => _error = 'فشل تنزيل التحديث. حاول مجدداً لاحقاً.');
    } finally {
      if (mounted) setState(() => _loading = false);
    }
  }
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
            ...
            child: Padding(
              ...
              child: Column(
                children: [
                  ...
                  Text('تحديثات وادنا', style: Theme.of(context).textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.w900)),
          ...
          else if (_error != null) _StatusCard(icon: Icons.error_outline_rounded, message: _error!, color: scheme.error, actionLabel: 'إعادة التحقق', onAction: _check)
          else if (update == null) _StatusCard(icon: Icons.cloud_off_outlined, message: 'لا تتوفر معلومات تحديث حالياً.', color: scheme.onSurfaceVariant, actionLabel: 'إعادة التحقق', onAction: _check)
          else if (!update.isUpdateAvailable) _StatusCard(icon: Icons.verified_outlined, message: 'تطبيقك محدّث. الإصدار الحالي: ${update.currentVersion}', color: Colors.teal, actionLabel: 'تحقق مجدداً', onAction: _check)
          else ...[
            ...
            if (AppUpdateService.isDirectDistribution) ...[
              if (_localApkPath == null)
                ...
                FilledButton.icon(
                  ...
                  icon: _installing ? const SizedBox(width: 19, height: 19, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white)) : const Icon(Icons.install_mobile_outlined),
              ...
              if (_downloadProgress != null) ...[
                ...
                Text('تنزيل: ${(_downloadProgress! * 100).round()}%', textAlign: TextAlign.center),
              ],
              if (!update.hasVerifiedDirectPackage) ...[
                const SizedBox(height: 12),
                const Text('سيصبح التنزيل متاحاً عند إعداد رابط APK آمن وبصمة SHA-256 في لوحة الإدارة.', textAlign: TextAlign.center),
              ],
            ] else
              FilledButton.icon(
            ...
            children: [
              Row(
                children: [
                  ...
                  Expanded(child: Text('يتوفر الإصدار ${update.latestVersion}', style: const TextStyle(fontWeight: FontWeight.w900))),
                  if (update.forceUpdate) const Chip(label: Text('مهم')),
                ],
              ),
              const SizedBox(height: 10),
              Text(update.releaseNotes?.isNotEmpty == true ? update.releaseNotes! : 'يتضمن هذا الإصدار تحسينات في الأداء والأمان وتجربة الاستخدام.'),
            ],
          ),
        ),
      );
...
class _StatusCard extends StatelessWidget {
  const _StatusCard({required this.icon, required this.message, required this.color, required this.actionLabel, required this.onAction});
  final IconData icon;
        ...
        child: Padding(
          ...
          child: Column(
            children: [
              ...
              const SizedBox(height: 12),
              Text(message, textAlign: TextAlign.center, style: const TextStyle(fontWeight: FontWeight.w700, height: 1.45)),
