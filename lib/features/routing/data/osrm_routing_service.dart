...
/// Direct, anonymous routing provider for visitor navigation.
///
/// It deliberately contains no secret. The optional Supabase/GraphHopper
/// adapter remains available for a future managed provider, while this adapter
/// lets public place coordinates be routed immediately.
class OsrmRoutingService implements RoutingService {
  ...
  Future<RoutingResult> calculateRoute({
    ...
    final uri =
        Uri.parse('$_baseUrl/${_profileFor(mode)}/$coordinates').replace(
      queryParameters: {
        'steps': 'true',
        'geometries': 'geojson',
        'overview': 'full',
    ...
    try {
      ...
      throw const RoutingException(
        'استغرق حساب المسار وقتاً أطول من المتوقع. تحقق من الاتصال ثم أعد المحاولة.',
      );
    } on http.ClientException {
      throw const RoutingException(
        'تعذر الاتصال بخدمة الملاحة. تحقق من اتصال الإنترنت ثم أعد المحاولة.',
      );
    } on FormatException {
      throw const RoutingException('تعذر قراءة بيانات المسار. حاول مجدداً.');
    } catch (_) {
      throw const RoutingException(
  ...
  Map<String, dynamic> _readPayload(http.Response response) {
    ...
    if (decoded is! Map) {
      throw const FormatException('OSRM payload is not an object.');
    }
